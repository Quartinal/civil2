...
class SearchBar
  extends EventEmitter<"submit" | "encode" | "decode">
  implements ISearchBar
{
  ...
  registerHandlers() {
    ...
    this.on("submit", (frame: HTMLIFrameElement, term: string) => {
      callFunctionOnContext(frame.contentWindow!, proxy.value.encodeUrl, term);
    });
